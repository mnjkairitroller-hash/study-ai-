        ...
        {rewards.map(reward => {
          ...
          return (
            <motion.div 
              ...
              key={reward.id} 
              className="app-card rounded-2xl border p-4 flex items-center gap-4 relative overflow-hidden"
            >
              ...
              <div>
                <button
                  ...
                  className={`px-4 py-2 rounded-xl font-bold text-sm shadow-sm transition-all ${
                    canAfford 
                      ? 'app-btn-primary hover:scale-105' 
                      : 'bg-black/10 dark:bg-white/10 opacity-50 cursor-not-allowed flex items-center gap-2'
                  }`}
                >
